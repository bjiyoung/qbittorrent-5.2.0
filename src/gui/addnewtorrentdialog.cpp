...
#define SETTINGS_KEY(name) u"AddNewTorrentDialog/" name
    ...
    const QString KEY_MINSIZEFILTER_ENABLED = SETTINGS_KEY(u"MinSizeFilterEnabled"_s);
    const QString KEY_MINSIZEFILTER_VALUE   = SETTINGS_KEY(u"MinSizeFilterValue"_s);
    const QString KEY_MINSIZEFILTER_UNIT    = SETTINGS_KEY(u"MinSizeFilterUnit"_s);
    ...
    , m_storeFilterPatternFormat {u"GUI/" SETTINGS_KEY(u"FilterPatternFormat"_s)}
{
    ...
    // Min-size filter row — inserted above contentFilterLayout inside verticalLayout_5
    m_minSizeCheckBox  = new QCheckBox(tr("최소 크기 :"), this);
    m_minSizeSpinBox   = new QSpinBox(this);
    m_minSizeUnitCombo = new QComboBox(this);

    m_minSizeSpinBox->setRange(0, 999999);
    m_minSizeSpinBox->setValue(0);
    m_minSizeSpinBox->setMinimumWidth(80);

    m_minSizeUnitCombo->addItem(u"KB"_s, Q_INT64_C(1024));
    m_minSizeUnitCombo->addItem(u"MB"_s, Q_INT64_C(1024) * 1024);
    m_minSizeUnitCombo->addItem(u"GB"_s, Q_INT64_C(1024) * 1024 * 1024);
    m_minSizeUnitCombo->setCurrentIndex(1); // default MB

    m_minSizeSpinBox->setEnabled(false);
    m_minSizeUnitCombo->setEnabled(false);

    auto *minSizeLayout = new QHBoxLayout();
    minSizeLayout->addWidget(m_minSizeCheckBox);
    minSizeLayout->addWidget(m_minSizeSpinBox);
    minSizeLayout->addWidget(m_minSizeUnitCombo);
    minSizeLayout->addStretch();

    // verticalLayout_5 owns contentFilterLayout at index 0 and contentTreeView at index 1
    m_ui->verticalLayout_5->insertLayout(0, minSizeLayout);

    // Wire up signals — save settings and re-apply filter on any change
    const auto onMinSizeChanged = [this]()
    {
        saveMinSizeFilterSettings();
        applyMinSizeFilter();
    };
    connect(m_minSizeCheckBox, &QCheckBox::toggled, this, [this, onMinSizeChanged](bool checked)
    {
...
void AddNewTorrentDialog::setupTreeview()
{
    ...
    // Apply min-size filter if enabled (adaptor now has real file data)
    applyMinSizeFilter();
...
void AddNewTorrentDialog::applyMinSizeFilter()
{
    if (!m_contentAdaptor)
        return;
    if (!m_minSizeCheckBox->isChecked())
        return;

    const qint64 multiplier = m_minSizeUnitCombo->currentData().toLongLong();
    const qint64 threshold  = static_cast<qint64>(m_minSizeSpinBox->value()) * multiplier;

    const int fileCount = m_contentAdaptor->filesCount();
    QList<BitTorrent::DownloadPriority> priorities = m_contentAdaptor->filePriorities();

    for (int i = 0; i < fileCount; ++i)
    {
        if (m_contentAdaptor->fileSize(i) >= threshold)
            priorities[i] = BitTorrent::DownloadPriority::Normal;
        else
...
{
    auto *s = SettingsStorage::instance();
    s->storeValue(KEY_MINSIZEFILTER_ENABLED, m_minSizeCheckBox->isChecked());
    s->storeValue(KEY_MINSIZEFILTER_VALUE,   m_minSizeSpinBox->value());
    s->storeValue(KEY_MINSIZEFILTER_UNIT,    m_minSizeUnitCombo->currentIndex());
}

void AddNewTorrentDialog::loadMinSizeFilterSettings()
{
    auto *s = SettingsStorage::instance();
    const bool enabled = s->loadValue<bool>(KEY_MINSIZEFILTER_ENABLED, false);
    const int  value   = s->loadValue<int>(KEY_MINSIZEFILTER_VALUE, 0);
    const int  unit    = s->loadValue<int>(KEY_MINSIZEFILTER_UNIT, 1); // default MB

    {
        const QSignalBlocker sb1 {m_minSizeSpinBox};
        const QSignalBlocker sb2 {m_minSizeUnitCombo};
        m_minSizeSpinBox->setValue(value);
        m_minSizeUnitCombo->setCurrentIndex(unit);
    }

    m_minSizeSpinBox->setEnabled(enabled);
    m_minSizeUnitCombo->setEnabled(enabled);

    // Trigger toggled() to apply filter and potentially start countdown
    m_minSizeCheckBox->setChecked(enabled);
}

void AddNewTorrentDialog::startOkCountdown()
{
    stopOkCountdown();
    ...
    connect(m_countdownTimer, &QTimer::timeout, this, [this]()
    {
        ...
        {
            btn->setText(tr("OK(%1)").arg(m_countdownValue));
        }
        else
        {
            stopOkCountdown();
...
void AddNewTorrentDialog::stopOkCountdown()
{
    if (m_countdownTimer)
    {
