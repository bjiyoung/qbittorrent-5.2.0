    ...
    , m_sortFilterModel {new TransferListSortModel(this)}
{
    ...
    // Auto-complete: stop torrents when download finishes and move (if any) completes
    connect(BitTorrent::Session::instance(), &BitTorrent::Session::torrentFinished
            , this, &TransferListWidget::handleTorrentFinishedForAutoComplete);
    connect(BitTorrent::Session::instance(), &BitTorrent::Session::torrentsUpdated
            , this, &TransferListWidget::handleTorrentsUpdatedForAutoComplete);
    connect(BitTorrent::Session::instance(), &BitTorrent::Session::torrentAboutToBeRemoved
            , this, [this](BitTorrent::Torrent *torrent) { m_pendingAutoComplete.remove(torrent); });
}
...
void TransferListWidget::handleTorrentFinishedForAutoComplete(BitTorrent::Torrent *torrent)
{
    if (!torrent)
        return;
    // If moving, defer stop until move completes
    if (torrent->isMoving())
        m_pendingAutoComplete.insert(torrent);
    else
        torrent->stop();
}

void TransferListWidget::handleTorrentsUpdatedForAutoComplete(const QList<BitTorrent::Torrent *> &torrents)
{
    if (m_pendingAutoComplete.isEmpty())
        return;
    for (BitTorrent::Torrent *torrent : torrents)
    {
        if (!m_pendingAutoComplete.contains(torrent))
            continue;
        if (!torrent->isMoving() && torrent->isFinished())
        {
            m_pendingAutoComplete.remove(torrent);
            torrent->stop();
        }
    }
}
